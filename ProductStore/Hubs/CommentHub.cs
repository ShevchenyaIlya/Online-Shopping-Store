...
namespace ProductStore.Hubs
{
    public class CommentHub : Hub
    {
        ...
        [Authorize]
        public async Task SendMessage(string title, string body, string image, string username, string productName)
        {
            if (!(productName == null || title == null || body == null))
            {
                ...
                await Clients.Group(productName).SendAsync("ReceiveMessage", title, body, dataComment.PostDate.ToString(), image, username, productName);
            }
        }
    }
}
